...
double LU_mpi_pipe(int argc, char* argv[]) {  //流水线的循环划分
    double start_time = 0;
    double end_time = 0;
    MPI_Init(&argc, &argv);
    int total = 0;
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    start_time = MPI_Wtime();
    int pre_proc = (rank + (total - 1)) % total;
    int next_proc = (rank + 1) % total;
    for (k = 0; k < N; k++) {
        if (k % total == rank) {
            for (j = k + 1; j < N; j++) {
                A[k][j] = A[k][j] / A[k][k];
            ...
            for (j = 0; j < total; j++) { //
                if (j != rank)
                    MPI_Send(&A[k][0], N, MPI_FLOAT, next_proc, 2, MPI_COMM_WORLD);//0号消息表示除法完毕
            }
        }
        else {
            int src = k % total;

            MPI_Recv(&A[k][0], N, MPI_FLOAT, pre_proc, 2,
                MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            if (int(k % total) != next_proc)
                MPI_Send(&A[k][0], N, MPI_FLOAT, next_proc, 2, MPI_COMM_WORLD);
        }
        int begin = k;
        while (begin % total != rank)
            begin++;
...
int main(int argc, char* argv[]) {
    ...
    // LU_mpi_plus(argc, argv);
    // LU_mpi_withExtra(argc, argv);
    // LU_mpi_pipe(argc, argv);
    return 0;
